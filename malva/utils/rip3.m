...
#path string con direccion a archivos de salida
	...
	for i= 1:28 #rows(dire)
	path= strcat(path_,"/", num2str(i));#char(Orden_(i)));
	path2= strcat(path2_,"/", num2str(i));#char(Orden_(i)))
	files= ls(path);
	Orden=cellstr(files);
	Orden=sort(Orden);
	Nvar= [];#cellstr(Head);
...
##data2
	...
	Aux= [i,Probc(i),Probm(i),NroInd(i),ProbD(i),mean(Nvar(:,3)),mean(Nvar(:,7)),mean(Nvar(:,4)),mean(Nvar(:,8))];
	Matrix=[Matrix;Aux];
#Nvar
	endfor
