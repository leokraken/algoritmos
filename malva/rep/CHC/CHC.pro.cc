...
//  Crossover:Intra_operator -------------------------------------------------------------
	...
	{
		 //unsigned long nbytes,length;
		 //_sc.get_contents_state_variable("_parameter_select_new_pop",(char *)probability,nbytes,length); MAS ESPANTOSO TODAVIA!
		 //para que actualizar el parametro de crossover? y si hay que actualizarlo por lo menos que sea con _crossover_probability
	}

	void Crossover::setup(char line[MAX_BUFFER])
	{
		int op;
		//sscanf(line," %d ",&op); !!ESPANTOSO!!
...
// Solver (superclasse)---------------------------------------------------
	...
	float *Solver::diverge_probability() const
	{
		float *current_probability = new float[MAX_PROB_PER_OP];
 		unsigned long nitems,length;
		_sc.get_contents_state_variable("_diverge_probability",(char *)&current_probability,nitems,length);
		return current_probability;
	}
	...
	float Solver::parameter_select_new_pop() const
	{
		...
		unsigned long nitems,length;
		_sc.get_contents_state_variable("_parameter_select_new_pop",(char *)&parameter_select_new_pop,nitems,length);
